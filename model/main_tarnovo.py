...
df4.drop(df4[(df4["price"] > 160000)].index, inplace=True)
...
dump(elasticnet_model, "tarnovo_appartament_price_model_elasticnet.joblib")
dump(lasso_model, "tarnovo_appartament_price_model_lasso.joblib")
dump(ridge_model, "tarnovo_appartament_price_model_ridge.joblib")
dump(gbr_model, "tarnovo_appartament_price_model_gbr.joblib")
dump(xgboost_model, "tarnovo_appartament_price_model_xgboost.joblib")
dump(stack_model, "tarnovo_appartament_price_model_stack.joblib")

# in order for our model to work we need to have the columns data
# that is why we need to export a json file
columns = {"data_columns": [col.lower() for col in X.columns]}
with open("tarnovo_columns.json", "w", encoding="utf-8") as f:

...
# # stack_train_score = stack_gen.score(X) # Calculating score
# # print('-score: %s' % stack_train_score)

print('elasticnet')
elasticnet_fit = elasticnet.fit(X, y)
# # #Training set performace
...
#     return ((0.1 * elasticnet_fit.predict(X)) + \
#             (0.1 * lasso_fit.predict(X)) + \
#             (0.1 * ridge_fit.predict(X)) + \
#             (0.1 * gbr_fit.predict(X)) + \
#             (0.2 * xgboost_fit.predict(np.array(X))) + \
#             (0.1 * lightgbm_fit.predict(X)) + \
#             (0.3 * stack_fit.predict(np.array(X))))
...
def predict_price(location,m2,rooms,floor,build):
    ...
    return inv_boxcox1p(stack_gen.predict([x])[0],0.05)
